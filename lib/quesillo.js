...
const $setCSS=(el,val)=>{if($(el)[0]==undefined){$(el).style = val}else{for(var i=0;i< $(el).length;i++){$(el)[i].style=val}}}
const $getCSS=(el,val)=>document.defaultView.getComputedStyle($(el), null).getPropertyValue(val);
...
const $ajax = (url,method,data,fxOk,fxError)=>{
    const xmlres = ()=>{
        if(connection.readyState == 4){
            if(connection.status == 200) fxOk(connection.response);
            else fxError(connection.response);
        }
    }
    ...
    connection.onreadystatechange = xmlres;
